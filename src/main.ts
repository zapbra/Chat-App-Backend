...
async function main() {
    try {
        ...
        io.on("connection", async (socket) => {
            ...
            socket.on("join dm", async (threadId: string) => {
                try {
                    ...
                    console.log("Error in 'join dm': ", error);
                    socket.emit("error", {
                        event: "join dm",
                        message: error instanceof Error ? error.message : error,
                    });
            ...
            socket.on("leave dm", async (threadId: string) => {
                try {
                    ...
                    console.log("Error in 'leave dm': ", error);
                    socket.emit("error", {
                        event: "leave dm",
                        message: error instanceof Error ? error.message : error,
                    });
            ...
            socket.on(
                ...
                async ({
                    ...
                    try {
                        ...
                        console.log("Error in 'dm message read': ", error);
                        socket.emit("error", {
            ...
            socket.on(
                ...
                async ({ senderId, receiverId, message, threadId }) => {
                    ...
                    try {
                        ...
                        if (newMessage) {
                            io.to(`thread:${threadIdResponse}`).emit(
                                "dm message",
                                {
                        ...
                        console.log("Error in 'dm message': ", error);
                        socket.emit("error", {
                            event: "dm message",
                            message:
                                error instanceof Error ? error.message : error,
            ...
            socket.on(
                ...
                async ({ roomId, senderId, username, message, replyId }) => {
                    try {
                        ...
                        if (createdMessage) {
                            const replyMessage =
                                await MessageDataService.getMessageById(
                                    replyId
                                );
                ...
                try {
                    ...
                    for (const roomId of socket.data.joinedRooms) {
                        ...
                        if (memberCount === 0) {
                            ...
                            const members = await pubClient.zRange(
                                roomKey,
                                0,
                                -1
                            );
        ...
        console.log("Fatail error in main():", err);
        process.exit(1);
    }
}
